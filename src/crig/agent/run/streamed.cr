module Crig
  ...
  class StreamedTurnAssembler
    ...
    def finish(message_id : String?, final_choice : OneOrMany(Completion::AssistantContent)) : StreamedTurn
      internal_call_ids = @pending_tool_calls.map { |tool_call, icid| {tool_call.id, icid} }
      ...
      choice = if !@pending_tool_calls.empty? || !@accumulated_reasoning.empty?
                 ...
                 tool_items = @pending_tool_calls.map { |tool_call, _| Completion::AssistantContent.tool_call(tool_call.id, tool_call.function.name, tool_call.function.arguments) }
...
module Crig
  def self.merge_reasoning_blocks(accumulated : Array(Completion::Reasoning), incoming : Completion::Reasoning)
    existing = accumulated.reverse_each.find { |reasoning| reasoning.id == incoming.id && reasoning.id && incoming.id }
    if existing
      existing.content.concat(incoming.content)
    else
  ...
  def self.ordered_streaming_assistant_content(
    ...
    reasoning_items.each { |reasoning| content_items << Completion::AssistantContent.reasoning(reasoning.display_text) }
  ...
  def self.assistant_text_items_from_choice(choice : OneOrMany(Completion::AssistantContent)) : Array(Completion::AssistantContent)
    choice.to_a.select { |content| content.kind.text? && content.text.try(&.text) != "" }
  end
end
