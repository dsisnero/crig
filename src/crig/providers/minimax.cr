  ...
  module Providers
    module MiniMax
      ...
      struct ClientBuilder
        ...
        def initialize(@api_key : String? = nil, @base_url : String = GLOBAL_API_BASE_URL)
        end
        ...
        def china : self
          base_url(CHINA_API_BASE_URL)
        end

        def build : Client
          key = @api_key || raise "MINIMAX_API_KEY not set"
      ...
      struct ApiResponse(T)
        ...
        def self.from_json_value(value : JSON::Any, & : JSON::Any -> T) : self
          if error = value.as_h["error"]?
            if message = error.as_h["message"]?.try(&.as_s?)
      ...
      struct Client
        ...
        def initialize(@api_key : String, @base_url : String = GLOBAL_API_BASE_URL)
        end
        ...
        def self.from_env : self
          api_key = ENV["MINIMAX_API_KEY"]? || raise "MINIMAX_API_KEY not set"
          base_url = ENV["MINIMAX_API_BASE"]? || GLOBAL_API_BASE_URL
          new(api_key, base_url)
        end

        def self.from_val(input : String) : self
          new(input, GLOBAL_API_BASE_URL)
        end
      ...
      struct CompletionModel
        ...
        def completion(request : Crig::Completion::Request::CompletionRequest)
          ...
          span.set_attribute(Crig::Telemetry::GEN_AI_PROVIDER_NAME, "minimax")
          ...
          payload = MiniMaxCompletionRequest.from_request(@model, request).to_json_value
          ...
          response_body = body.ok || raise Crig::Completion::CompletionError.new("MiniMax response did not include a success payload")
        ...
        def stream(request : Crig::Completion::Request::CompletionRequest)
          request_payload = MiniMaxCompletionRequest.from_request(@model, request)
          ...
          payload = MiniMaxCompletionRequest.new(
          ...
          if response.status_code >= 400
            ...
            raise Crig::Completion::CompletionError.new(response.body)
          end

          body_io = response.body_io?
          raw_choices, final_usage =
            if body_io
              parse_streaming_choices(body_io)
            else
              parse_streaming_choices(IO::Memory.new(response.body))
            end
        ...
        private def parse_streaming_choices(io : IO) : {Array(Crig::RawStreamingChoice(Crig::Client::FinalCompletionResponse)), Crig::Completion::Usage?}
      ...
      struct Client
        include Crig::CompletionClient(Crig::Providers::MiniMax::CompletionModel)
      end

      # Anthropic-compatible API path
      struct MiniMaxAnthropicExt
      end

      struct MiniMaxAnthropicBuilder
      ...
      struct AnthropicClientBuilder
        ...
        def initialize(
          @api_key : String,
          @base_url : String = GLOBAL_ANTHROPIC_API_BASE_URL,
        ...
        def china : self
          base_url(CHINA_ANTHROPIC_API_BASE_URL)
        end
      ...
      struct AnthropicClient
        ...
        def self.from_env : self
          ...
          if primary = ENV["MINIMAX_ANTHROPIC_API_BASE"]?
            builder = builder.base_url(primary)
          elsif fallback = ENV["MINIMAX_API_BASE"]?
            builder = builder.base_url(MiniMax.normalize_anthropic_base_url(fallback))
          end
      ...
      def self.normalize_anthropic_base_url(base_url : String) : String
        ...
        if normalized == GLOBAL_API_BASE_URL.rstrip('/')
          GLOBAL_ANTHROPIC_API_BASE_URL
        elsif normalized == CHINA_API_BASE_URL.rstrip('/')
          CHINA_ANTHROPIC_API_BASE_URL
        elsif normalized.ends_with?("/anthropic")
          normalized
