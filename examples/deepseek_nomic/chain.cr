...
module Crig::Examples::DeepSeekNomicChain
...
if PROGRAM_NAME == __FILE__
  ...
  if deepseek_api_key
    ...
    store = Crig::Examples::DeepSeekNomicChain.build_store(embedding_model)
    ...
    agent = Crig::Examples::DeepSeekNomicChain.build_agent(deepseek_client)
    puts "   ✓ DeepSeek agent ready"

    # Build chain
    puts "4. Building RAG chain..."
    chain = Crig::Examples::DeepSeekNomicChain.build_chain(store, agent)
    puts "   ✓ Chain built successfully"

    # Run chain
    puts "5. Running chain with prompt: \"#{Crig::Examples::DeepSeekNomicChain.default_prompt}\""
    puts "=" * 60
    result = chain.call(Crig::Examples::DeepSeekNomicChain.default_prompt)
    ...
    puts "  crystal run examples/deepseek_nomic/chain.cr"
    puts ""
