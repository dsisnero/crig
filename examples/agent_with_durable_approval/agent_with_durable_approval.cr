...
# Ported from vendor/rig/examples/agent_with_durable_approval/src/main.rs
#
# Durable human-in-the-loop tool-call approval using serializable AgentRun.
# The entire run is JSON-serialized between approval rounds — the state
# machine carries no live state, only serializable data. This mirrors
# LangGraph's interrupt() + checkpointer pattern.
#
# Requires DEEPSEEK_API_KEY.

# Two tools: one read-only, one side-effecting.
...
struct GetBalance
  ...
  def definition(prompt : String) : Crig::Completion::ToolDefinition
    Crig::Completion::ToolDefinition.new("get_balance", "Get the balance of an account.", JSON.parse(%({
      "type": "object",
...
run = Crig::AgentRun.new(Crig::Completion::Message.user(prompt)).max_turns(10)
...
loop do
  ...
  in .call_tools?
    # Durable pause: serialize the run, then reconstruct from file.
    File.write(state_path, run.to_json)
    puts "\n💾 run suspended to #{state_path}"

    # --- imagine process exits here ---
    ...
    calls.each do |call|
      ...
      when "e", "edit"
        edited = ask("     replacement JSON args (single line): ")
        begin
          value = edited ? JSON.parse(edited) : nil
          if value
            output = ts_handle.call_tool(name, value.to_json)
            results << Crig::Completion::UserContent.tool_result(
              id,
              Crig::OneOrMany(Crig::Completion::ToolResultContent).one(
                Crig::Completion::ToolResultContent.text(output),
            ...
            puts "     ! no valid JSON; denying"
            results << Crig::Completion::UserContent.tool_result(
              id,
              Crig::OneOrMany(Crig::Completion::ToolResultContent).one(
                Crig::Completion::ToolResultContent.text("denied: no JSON"),
              ),
            )
          end
        rescue ex : JSON::ParseException
          puts "     ! invalid JSON; denying"
          results << Crig::Completion::UserContent.tool_result(
            id,
            Crig::OneOrMany(Crig::Completion::ToolResultContent).one(
              Crig::Completion::ToolResultContent.text("denied: invalid JSON"),
            ),
          )
        end
      when "b", "abort", nil
        ...
        results << Crig::Completion::UserContent.tool_result(
          id,
          Crig::OneOrMany(Crig::Completion::ToolResultContent).one(
            Crig::Completion::ToolResultContent.text(reason),
          ),
        )
      end
    end
