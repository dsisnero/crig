...
describe "StreamedTurnAssembler" do
  it "accumulates text and emits ingested events" do
    ...
    t1.ingest(Crig::StreamedAssistantContent(Crig::PromptResponse).text("hel")).size.should eq(1)
  end

  it "text accumulates across calls" do
    s = Set(String).new
    t1 = Crig::StreamedTurnAssembler.new(s, s)
    t1.ingest(Crig::StreamedAssistantContent(Crig::PromptResponse).text("hel"))
    t1.ingest(Crig::StreamedAssistantContent(Crig::PromptResponse).text("lo"))
  ...
  it "ingest handles reasoning items" do
    ...
    t1.ingest(Crig::StreamedAssistantContent(Crig::PromptResponse).reasoning(r)).size.should eq(1)
  end
  ...
  it "returns InvalidToolCall for disallowed tool" do
    ...
    events = t1.ingest(Crig::StreamedAssistantContent(Crig::PromptResponse).tool_call(tc, "internal_1"))
  ...
  it "finish passes raw choice through for plain text" do
    s = Set(String).new
    t1 = Crig::StreamedTurnAssembler.new(s, s)
    t1.ingest(Crig::StreamedAssistantContent(Crig::PromptResponse).text("hi"))
  ...
  it "finish orders reasoning then text then tool calls" do
    ...
    t1 = Crig::StreamedTurnAssembler.new(allowed.to_set, allowed.to_set)
    r = Crig::Completion::Reasoning.new([Crig::Completion::ReasoningContent.summary("think")], "r1")
    t1.ingest(Crig::StreamedAssistantContent(Crig::PromptResponse).reasoning(r))
    fn = Crig::Completion::ToolFunction.new("add", JSON.parse(%({"x":1})))
    tc = Crig::Completion::ToolCall.new("call_1", fn)
    t1.ingest(Crig::StreamedAssistantContent(Crig::PromptResponse).tool_call(tc, "internal_1"))
    t1.ingest(Crig::StreamedAssistantContent(Crig::PromptResponse).text("answer"))
