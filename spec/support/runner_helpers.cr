...
class RunnerMockModel
  ...
  def completion(request : Crig::Completion::Request::CompletionRequest) : Crig::Completion::CompletionResponse(String)
    ...
    elsif !@tool_call_names.empty? && @call_count == 1
      items = @tool_call_names.map_with_index { |n, i|
        Crig::Completion::AssistantContent.tool_call("call_#{i + 1}", n, JSON.parse(@tool_call_args))
      }
      return Crig::Completion::CompletionResponse(String).new(
        Crig::OneOrMany(Crig::Completion::AssistantContent).many(items), @usage, "raw")
    end
    choice = Crig::OneOrMany(Crig::Completion::AssistantContent).one(
  ...
  def completion_request(prompt : Crig::Completion::Message | String) : Crig::Completion::Request::CompletionRequestBuilder
    ...
    @tool_call_names.each do |n|
      builder = builder.tool(Crig::Completion::ToolDefinition.new(n, "test tool", JSON::Any.new({} of String => JSON::Any)))
    end
    builder
  end
end
