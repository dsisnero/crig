...
describe "OpenRouter response image filtering" do
  it "is_openrouter_response_image? detects tagged images" do
    ...
    Crig::Providers::OpenRouter.is_openrouter_response_image?(image).should be_true
  end

  it "is_openrouter_response_image? returns false for untagged images" do
    image = Crig::Completion::Image.new(
      Crig::Completion::DocumentSourceKind.new(Crig::Completion::DocumentSourceKind::Kind::Base64, string_value: "abc"),
      Crig::Completion::ImageMediaType::PNG,
    )
    Crig::Providers::OpenRouter.is_openrouter_response_image?(image).should be_false
  end

  it "is_openrouter_response_image? returns false for nil additional_params" do
    image = Crig::Completion::Image.new(
